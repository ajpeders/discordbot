  ...
  return (
    <div ref={wrapRef} className="relative flex-1">
      ...
      {open && error && (
        <div
          role="alert"
          className="absolute left-0 right-0 top-full z-20 mt-2 rounded-md border border-danger/40 bg-bg-elev px-3 py-2 text-xs text-danger"
        >
          {error}
        </div>
      )}

      {open && !error && searched && results.length === 0 && (
        <div
          role="status"
          className="absolute left-0 right-0 top-full z-20 mt-2 rounded-md border border-border bg-bg-elev px-3 py-2 text-xs text-muted"
        >
      ...
      {open && !error && results.length > 0 && (
        <ul
          id={listId}
          role="listbox"
          className="absolute left-0 right-0 top-full z-20 mt-2 max-h-96 overflow-y-auto rounded-lg border border-border bg-bg-elev shadow-2xl"
        >
          {results.map((r, i) => (
            <li key={r.url} role="option" aria-selected={i === hi}>
              ...
              >
                {r.thumbnail ? (
                  ...
                  <div className="flex h-10 w-16 shrink-0 items-center justify-center rounded bg-surface text-base text-dim">
                    ♫
                  </div>
                )}
      ...
      {loading && !open && trimmed.length >= 2 && !isUrl && (
